...
DPO_DIR       = "output/qwen-coder-finetune-dpo-v2"
DATA_PATH     = "data/github_solutions.json"
OUTPUT_PATH   = "evaluation/compare_v4_v5_dpo_v2.json"
...
def load_model(adapter_dir: str, label: str):
    """
    v4, v5는 base 모델 위에 LoRA 어댑터 1개를 얹는다.
    DPO는 v5 기반으로 학습했으므로 base -> v5 어댑터 -> dpo 어댑터 순으로 얹는다.
    """
    ...
    if label == "dpo":
        # v5 어댑터를 먼저 얹고, 그 위에 DPO 어댑터를 얹는다
...
def main():
    ...
    for version, adapter_dir in [("v5", V5_DIR), ("dpo_v2", DPO_DIR)]:
        ...
        for i, problem in enumerate(problems):
            ...
            if entry is None:
                entry = {"title": title, "level": level, "sig": sig, "v4": {}, "v5": {}, "dpo": {}}
            ...
            for task in ["hint", "solution"]:
                ...
                if task == "solution":
                    ...
                    # 코드 완전 정답은 자동 채점이 어려워 수동 확인용으로 응답만 저장
                    entry[version]["code_correct"] = None  # 수동으로 True/False 채워서 채점
    ...
    print("\n주의: code_correct 필드는 null로 저장됨. evaluation/compare_v4_v5_dpo.json을")
