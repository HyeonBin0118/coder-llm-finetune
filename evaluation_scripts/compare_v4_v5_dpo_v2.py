...
def load_model(adapter_dir: str, label: str):
    """
    v5는 base 모델 위에 LoRA 어댑터 1개를 얹는다.
    DPO v2는 v5 기반으로 학습했으므로 base -> v5 어댑터 -> dpo_v2 어댑터 순으로 얹는다.
    """
    ...
    if label == "dpo_v2":
        # v5 어댑터를 먼저 얹고, 그 위에 DPO v2 어댑터를 얹는다
...
def main():
    ...
    for version, adapter_dir in [("v5", V5_DIR), ("dpo_v2", DPO_DIR)]:
        ...
        for i, problem in enumerate(problems):
            ...
            if entry is None:
                entry = {"title": title, "level": level, "sig": sig, "v5": {}, "dpo_v2": {}}
    ...
    print("\n주의: code_correct 필드는 null로 저장됨. evaluation/compare_v4_v5_dpo_v2.json을")
